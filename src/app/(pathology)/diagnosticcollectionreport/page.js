...
export function TestWiseCollectionReport() {
    return (
        <LayoutForm>
            <Heading headingText="Diagnostic Collection Report" />
...
const TestWiseCollectionReportform = () => {
    ...
    // fetch test
    // fetch test
    const fetchTest = async (catagorayId) => {
        try {
            const response = await apiClient.get(`testmaster/getBy/catagoryId?catagorayId=${catagorayId}`);
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-2 m-2">
                        <div className="flex flex-col sm:flex-row sm:items-center mb-4">
                            <div className="w-full sm:w-[20%] mb-2 sm:mb-0">
                                <label className="block font-semibold">Test Category</label>
                            </div>
                            <div className="w-full sm:w-[80%]">
                                <select
                                    ...
                                    onChange={(e) => {
                                        ...
                                        formik.setFieldValue('testCategoryId', selectedCategoryId);
                                        if (selectedCategoryId) {
                                            fetchTest(selectedCategoryId);
                                        } else {
                                            setTest([]);
                                        }
                                    }}
                                    className={`w-full px-4 py-2 border rounded-lg focus:outline-none ${formik.touched.testCategoryId && formik.errors.testCategoryId ? 'border-red-500' : ''}`}
                                    value={formik.values.testCategoryId}
                                >
                                    <option value="" label="Select category" />
                                    {category.map((category) => (
                                        <option key={category.testCategoryId} value={category.testCategoryId}>{category.testCategoryName}</option>
                                    ))}
                                </select>
                                {formik.touched.testCategoryId && formik.errors.testCategoryId && (
                                    <div className="text-red-500 text-sm">{formik.errors.testCategoryId}</div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center mb-4">
                            ...
                            <div className="w-full sm:w-[80%]">
                                <select
                                    name="testId"
                                    onChange={(e) => formik.setFieldValue('testId', e.target.value)}
                                    className={`w-full px-4 py-2 border rounded-lg focus:outline-none ${formik.touched.testId && formik.errors.testId ? 'border-red-500' : ''}`}
                                ...
                                {formik.touched.testId && formik.errors.testId && (
                                    <div className="text-red-500 text-sm">{formik.errors.testId}</div>
                                )}
                            </div>
                        </div>
