...
const DeductionForm = () => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      fetchData();
      setInputs({
        salaryId: 0, depName: "", empName: "", deptId: 0, empCode: 0, headName: "", amount: 0, forMonth: "", date: ""
      });
    } catch {
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-sky-50 via-white to-sky-50 rounded-xl shadow-xl border border-sky-100 ml-6 mt-6">
      ...
      <form onSubmit={handleSave} className="space-y-6 mt-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <select
            ...
            className="border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-sky-300"
          >
            <option value="">Select Employee</option>
            {emp.map((e) => (
              <option key={e.empCode} value={e.empCode}>{e.empName}</option>
            ))}
          </select>
          ...
          >
            ...
            {dep.map((d) => (
              <option key={d.deptId} value={d.deptId}>{d.depName}</option>
            ))}
          </select>
      ...
      <div className="mt-8 bg-white rounded-lg shadow-md border border-sky-100 overflow-auto max-h-96">
        <table className="w-full border-collapse text-center">
          <thead className="sticky top-0 bg-gradient-to-r from-sky-50 to-sky-100 backdrop-blur text-xs text-sky-700">
            <tr>
