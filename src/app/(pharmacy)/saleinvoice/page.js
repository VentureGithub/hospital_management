...
const SaleForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const response = await apiClient.post('/itemSale/saveData', dataToBeSend);
    ...
    const fetchAllApi = async () => {
        const response = await apiClient.get(`itemSale/getAllData`)
        console.log(response.data.data)
        if (response.status === 200) {
        ...
        <div className='p-4 bg-gradient-to-br from-sky-50 via-white to-sky-50 mt-6 ml-6  rounded-xl shadow-2xl border border-sky-100'>
            <div className="flex items-center justify-between border-b border-sky-100 pb-3">
                <div className="flex items-center gap-3">
                    <Heading headingText="Sale Invoice" />
                </div>
                <div className="text-xs text-sky-700 bg-sky-50 px-3 py-1 rounded-md border border-sky-100">Pharmacy • Billing</div>
            </div>
            <div className="py-2 ">
                <form onSubmit={handleSubmit} >
                                ...
                                <div className="w-full" style={{ maxHeight: "400px", overflowY: "auto" }}>
                                    <table className="table-auto w-full border border-collapse shadow">
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {tableData.map((item) => (
                                                <tr key={item.id}>
                                                    <td className="border border-gray-300 p-2">{item.medicineNames}</td>
                                                    <td className="border border-gray-300 p-2">{item.batchNo}</td>
                                                    <td className="border border-gray-300 p-2">{item.expiryDates}</td>
                                                    <td className="border border-gray-300 p-2">{item.quantity}</td>
                                                    <td className="border border-gray-300 p-2">{item.packetOrPcs}</td>
                                                    <td className="border border-gray-300 p-2">{item.rate}</td>
                                                    <td className="border border-gray-300 p-2">{item.quantity * item.rate}</td>
                    ...
                    <div className="mb-4 border  bg-white rounded-md p-2 ">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="w-full mt-1 text-sm px-3 py-2 bg-gray-100 border border-gray-300 rounded-md font-semibold"
                                            readOnly
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="w-full mt-1 px-3 py-2 bg-gray-100 border border-gray-300 rounded-md font-semibold text-red-600"
                                            readOnly
                                        />
                                    </div>
                                </div>
                        ...
                        <button
                            type="submit"
                            className="bg-blue-500 text-white text-sm px-6 py-2 rounded-md hover:bg-blue-600"
                        >
                            Submit Sale
                        </button>
...
export default withAuth(Sale, ['DOCTOR', 'ADMIN', 'SUPERADMIN'])
